...
$\DEdr(K{=}50) = 1.000$, above the two-real-cell noise ceiling ($0.76$) and above the model
($0.73$). The ordering is inverted: less information yields a higher score, which is the signature
of an artifact rather than of skill. The cause is control regression-to-the-mean: $\DEdr$ correlates
...
\midrule
\texttt{revert\_center} (all genes $\rightarrow P/2$) & none & \textbf{1.000} & n/a \\
...
\toprule
Layer & Drug decode & Ablate drug (KL) & Random, matched dim & Ratio & \textbf{Swap vs matched noise} \\
\midrule
2  & 0.354 & 0.155 & 0.016 & $9.4\times$ & \textbf{0.106 $<$ 0.916} \\
4  & 0.429 & 1.563 & 0.065 & $24\times$  & \textbf{1.805 $<$ 2.150} \\
6  & 0.529 & 1.435 & 0.120 & $12\times$  & \textbf{1.593 $<$ 1.957} \\
8  & 0.569 & 0.596 & 0.125 & $4.8\times$ & \textbf{0.593 $<$ 1.184} \\
9  & 0.602 & 0.418 & 0.037 & $11\times$  & \textbf{0.334 $<$ 0.760} \\
12 & 0.873 & 0.133 & 0.034 & $3.9\times$ & \textbf{0.107 $<$ 0.233} \\
16 & 0.883 & 0.015 & 0.005 & $3.2\times$ & \textbf{0.031 $<$ 0.036} \\
\bottomrule
\end{tabular}
\caption{All seven layers pass the removal gate (88--95\% of above-chance drug signal removed).
The swap is the identity test, and it is null at every depth.}
\label{tab:workspace}
\end{table}
...
not order itself by swap dissimilarity. The single-cell control, scored through the identical
pipeline, stays null --- so the result is not manufactured by the scoring path.}
...
\pending{\texttt{variance\_decomposition.py} is running.}

The two lookup arms point in opposite directions and $\NIR$ cannot adjudicate between them.
\texttt{drug\_lookup\_1} sits $0.136$ below the ceiling despite using \emph{more} cells than the
ceiling's half-sample, which argues for real cell-line-specific structure. Yet averaging many lines
recovers to $0.963$, and averaging removes only \emph{noise}, never the systematic miss of the
target line's own interaction --- so a large interaction should have left \texttt{drug\_lookup} on a
plateau well below the ceiling.

The question is therefore posed in cosine space with the noise divided out
(Section~\ref{sec:methods-vardecomp}). Validation shows why this is necessary: at $\kappa = 0$ the
raw cross-line cosine is $0.734$, which read naively reports $27\%$ interaction where the truth is
zero.

\begin{description}
  \item[If $\Tcoef$ is near its $\kappa = 0$ null] the drug-specific residual is, to measurement
        precision, a per-drug constant. A lookup table is then the correct model for this task; the
        failure of the consensus arm, the OT arm and the whole $\varepsilon$-ladder is
        \emph{structural} rather than an engineering shortfall; and the thesis closes with a
        quantitative statement of why conditional generation cannot help.
  \item[If $\Tcoef$ falls well below the null] a real drug$\times$cell-line interaction exists, the
        lookup's $0.963$ is an artifact of $\NIR$ compression, and there is a conditional target that
        no lookup can structurally reach.
\end{description}

The same run reports $\lVert\resid\rVert / \lVert\shift\rVert$, the fraction of the total
perturbation response that the drug-specific frame covers, which scopes every claim made above.
