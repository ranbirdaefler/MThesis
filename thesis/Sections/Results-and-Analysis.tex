...
including the cell-line directions this same experiment measures at $\approx 0.6$ variance share
against the drug subspace's $0.03$, and up to $11\times$ its ablation KL. ``The swap moves the output
less than matched noise'' is precisely what that
...
$0.333$ vs $0.334$; layer 16: $0.031$ vs $0.031$), ruling out estimation noise as the cause.

\paragraph{Validity of the null, and how much of it survives.} Two pillars stand and one does not.
The removal gate passes at every layer, so the nulls are not broken hooks. The cell-line positive
control carries $\approx 20\times$ the purified drug subspace's variance share ($0.6$ against
$0.03$) and up to $11\times$ its ablation KL, so the instrument does register a strong effect where
one exists --- but that KL ratio is not uniform, and at layer 12 it falls to $1.02\times$, meaning
the instrument has essentially no headroom at that depth and no claim should rest on it. The third
pillar, that the swap is translation-clean by matched norm, does not survive: as described above,
the norm was matched but the subspace was not.
...
representation}. That pattern is what the capacity-allocation account of
Section~\ref{sec:lit-interp} predicts~\cite{superposition}: cell-line identity is consulted on every
forward pass, because emitting any plausible profile requires it, and so earns dedicated causal
weight; drug$\rightarrow$gene response is needed only for context-dependent composition, and can be
encoded ever more explicitly without the readout ever querying \emph{which} drug it names. The
reading is ours, not theirs --- we are applying a claim about how capacity is allocated in language
models to a case it was not derived from --- and it earns its place here by being falsifiable rather
than by being borrowed. It predicts the next result: if the defect is a readout that ignores
direction, no change to the target can touch it.
