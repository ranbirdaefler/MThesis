...
\paragraph{Held-out pairings score as well as trained ones} ($+0.1002$ vs $+0.0898$, intervals
almost entirely overlapping), which is consistent with a drug representation that applies in cell
lines where the drug was not observed rather than with memorised pairings. Two qualifications
belong with the number. The targets these were measured against were built with the generic fitted
over every condition, holdout included, so the held-out arm was scored against targets partly
defined by the holdout. And a held-out (drug, cell line) pairing can share a treated well with a
training condition of another cell line, because one Tahoe well carries many lines. Both are
addressed in the rebuild; until it lands this is evidence for the claim, not a settled measurement
of it.
