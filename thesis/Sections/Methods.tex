...
\paragraph{Backbone.} \texttt{C2S-Scale-Pythia-1b-pt}~\cite{c2sscale}, a C2S-Scale checkpoint built
on the Pythia-1b decoder~\cite{pythia}, cold-started for every arm from the same base checkpoint with
...
the tier-2 and tier-3 splits to mean what they claim: a drug held out here is unseen by the whole
pipeline, not merely by its final stage.
