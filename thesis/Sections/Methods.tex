...
\paragraph{Normalised Inverse Rank.} For a prediction $\hat{y}$ of condition $i$, with truths
$\{y_j\}$ over the comparison set,
\begin{equation}
...
\paragraph{Metric calibration (\DRF).} Following Miller et al.~\cite{miller}, each metric $m$ is
graded rather than each model:
\begin{equation}
  \DRF(m) \;=\; \frac{m(\textrm{pos}) - m(\textrm{neg})}{m(\textrm{perfect}) - m(\textrm{neg})},
\end{equation}
with $\textrm{pos}$ an interpolated-duplicate noise ceiling and $\textrm{neg}$ a drug-agnostic
leave-one-out mean. A negative \DRF{} means the metric scores the uninformative baseline
\emph{above} the noise ceiling --- it is inverted, and rewards the generic gene ordering.
...
lines}, not conditions --- drugs within a cell line are pseudoreplicates, and an observation-level
interval would be badly anti-conservative. For the transfer coefficient
...
A three-way split, keyed at the \textbf{(drug, cell line)} level so that every plate and dose of a
held-out pair goes to the held-out split and no sibling condition leaks into training:
\begin{description}
  \item[\texttt{unseen\_drug}] every condition of a held-out drug. The model never sees the token.
        This is the \emph{control}: given that chemical structure does not predict response
        (Section~\ref{sec:res-scope}), it should fail, and its failing is what proves that any
        transfer observed in \texttt{unseen\_combo} comes from having seen the drug.
  \item[\texttt{unseen\_combo}] a held-out (drug, cell line) pair whose drug is seen in at least
        three other cell lines. This is cross-context transfer and the scientifically meaningful
        test.
...
conditional model can win on. $\Tcoef$ is estimated by disattenuated cross-line correlation,
\begin{equation}
