...
run or is stored in a result artifact. The $\Tcoef$ rows reproduce exactly from
\texttt{variance\_decomposition.py --selftest} and the channel-gate rows from
\texttt{channel\_gate.py --selftest}. The $\kappa$-structure rows are the exception: they come from
a one-off planted-world run of \texttt{kappa\_structure()}, which the current code exposes only as a
real-data flag and not as a selftest, and are \pending{reproducible rerun}.}
\label{tab:planted}
\end{table}
...
A third defect was found after the canonical run and is not yet reflected in any channel number.
The gate's \emph{retrieval} pool --- the partners whose mean residual forms the prediction --- was
...
unrestricted pool; the rerun has not been performed. Every channel-gate figure measured on Tahoe in
this thesis therefore predates the fix --- the planted worlds above are a separate synthetic path
with no held-out split and are unaffected --- and any claim that unseen-drug generalisation is
reachable by retrieval over target or mechanism is suspended pending that rerun.
