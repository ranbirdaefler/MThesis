...
bottleneck this work identifies is not capacity. It is that the target tokens are overwhelmingly
...
very little of the training signal can be carrying drug identity. No token-level loss or gradient
attribution was computed, so no fraction of the gradient itself is claimed; a larger model receives the same diluted signal from the same
