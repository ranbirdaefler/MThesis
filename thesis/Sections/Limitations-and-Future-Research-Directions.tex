...
generations per arm at temperature $0.8$ that second source is not negligible, and no across-seed
replication of the generation arms was run --- not deferred, not planned. Every interval quoted
...
is not distinguishable from zero. A mechanism-only lookup says the same thing from the other side:
pooled over the $n = 218$ conditions on which both are scored, the model is ahead of it by $+0.044$
($[-0.048, +0.135]$ clustered two-way on cell line and well), so nothing in this comparison
shows the class label on its own buying more than the model already has.
...
fine-tuning only. Pythia-1b saw drug names in natural-language pretraining, and the prompt still
supplies \texttt{Mechanism}.
