...
of several estimates changed and not only their magnitude. Two components are still pending re-run
and are marked where they are used: the $\DRF$ calibration bootstrap (\S\ref{sec:res-metrics})
and the channel-gate partner pool (\S\ref{sec:res-scope}).
...
(the same-plate run carries the identical defect over its $335$ rows). The one-sided $p$ was read
...
family was in fact tested. Both are repaired in the analysis code and the rerun is pending. Until
it lands, the interval and the Holm $p$ quoted for $\DRF$ below are withdrawn and only the point
estimates stand.

Under the hardened protocol, $\NIR$ is the only one of the five whose $\DRF$ is positive. The
comparison is reported descriptively, without an interval or a $p$-value, because the stored
calibration artifact cannot support inference: its bootstrap resampled the $1{,}820$ drug rows as
though each were a cell line, when the run spans $25$, and the resulting one-sided $p$ sits exactly at
the resampling floor of $1/2001$ rather than at an estimated value. A corrected re-run is pending and
only the point estimates are quoted here. In the all-plates run ($1{,}820$ drugs over $25$ cell
lines), $\NIR$ reads $+0.635$ against $-0.163$ (\texttt{weighted\_r2}), $-0.319$ ($\paneltau$),
$-0.650$ (\texttt{spearman\_expr}) and $-0.686$ ($\DEdr$). Restricting swap partners to the same
plate ($335$ drugs over the same $25$ cell lines) pulls every value toward zero and changes no sign:
$\NIR$ $+0.519$, against $-0.122$, $-0.247$, $-0.495$ and $-0.500$ in the same order. What the
argument uses is the sign pattern, which is identical in both runs; no ranking among the four
...
shows the same-plate run ($335$ drugs over $25$ cell-line$\times$plate groups), the confound-free
version, in which $\NIR$ reads $+0.519$ against $-0.122$ (\texttt{weighted\_r2}), $-0.247$
($\paneltau$), $-0.495$ (\texttt{spearman\_expr}) and $-0.500$ ($\DEdr$); the all-plates run
($1{,}820$ drugs over $25$ cell lines) gives $+0.635$ for $\NIR$, with the four rivals between
$-0.163$ and $-0.686$. Point estimates only, with no intervals drawn: the stored bootstrap treated
each drug row as its own cluster, so its intervals and $p$-values are withheld pending a corrected
re-run (see text). \texttt{weighted\_r2} carries no directional claim: its sign tracks cell count.}
...
of the five candidates audited, $\NIR$ is the only one with a positive $\DRF$ ($+0.635$ all-plates,
$+0.519$ with same-plate partners; point estimates only --- the stored intervals and $p$-values are
invalid, and a corrected re-run is pending); and on
...
test the optimal-transport arm does show a weak effect (\ci{+0.0263}{+0.0069}{+0.0476}). That
...
% AUTHOR NOTE: the 62% / 19% pair cited above IS sourced -- RESULTS_cluster/target_divergence.json,
% scopes.cellline.reliability.frac_reproducible = 0.6169 and scopes.plate... = 0.1933, both at thr 0.2.
% The disjoint-control pair has NO json artifact in RESULTS_cluster. Two prose records disagree:
% FINDINGS.md:546 gives 20% plate / 16% cell line; coda_errata/ERRATA.md:985-988 quotes a real-cache
% rebuild log giving 19% plate / 16% cell line at thr 0.2 (and 51% / 47% at the resolved thr 0.1086).
% Restore digits only once a rerun writes them to an artifact.
...
% AUTHOR NOTE: no artifact (the mechanism-matched permutation p 0.15-0.31 quoted in an earlier
% draft). Withdrawn, not restated; rerun or cut the clause before submission.
...
\texttt{train} & 300 (39; 76; 136) & 0.598 [0.549, 0.647] & \ci{+0.0898}{+0.0382}{+0.1415} & \ci{+0.0898}{+0.0291}{+0.1506} \\
\textbf{\texttt{unseen\_combo}} & \textbf{895 (42; 34; 36)} & \textbf{0.533 [0.492, 0.575]} & \ci{+0.0332}{+0.0069}{+0.0594} & $+0.0332$ [$-0.0031$, $+0.0695$] \\
\texttt{unseen\_drug} & 199 (40; 10; 28) & 0.459 [0.404, 0.514] & $-0.0315$ [$-0.0836$, $+0.0206$] & $-0.0315$ [$-0.1049$, $+0.0419$] \\ 0.598 [0.549, 0.647] & \ci{+0.0898}{+0.0382}{+0.1415} & \ci{+0.0898}{+0.0291}{+0.1506} \\
\textbf{\texttt{unseen\_combo}} & \textbf{895 (42; 34; 36)} & \textbf{0.533 [0.492, 0.575]} & \ci{+0.0332}{+0.0069}{+0.0594} & $+0.0332$ [$-0.0031$, $+0.0695$] \\
\texttt{unseen\_drug} & 199 (40; 10) & 0.459 [0.404, 0.514] & $-0.0315$ [$-0.0836$, $+0.0206$] & $-0.0315$ [$-0.1049$, $+0.0419$] \\
\bottomrule
\end{tabular}}
...
\label{tab:lookup}
\end{table}
% AUTHOR NOTE (run C): canonical moa_lookup = 0.5520 on n = 218 (re_v3.json means.moa_lookup;
% n from means.vs_baselines.moa_lookup -- verified same support: exactly 218 records carry both
% model and moa_lookup). Paired model - moa_lookup = +0.0436, ci_two_way [-0.0480, +0.1352],
% which spans zero, so the comparison stays uninformative rather than favourable.
% STILL STALE (run B) IN THIS BLOCK: table row says n = 118 / 0.565, caption says 118, prose says
% +0.0413 [-0.0610, +0.1436]. Do not delete this note until those three are updated.
...
every retained condition in the cell line; the restriction is now in the code and the rerun is
pending, so every verdict in this subsection, and the claim below, is reported subject to that
rerun. The gate's estimand requires the channel arm
...
\midrule
\textbf{protein target} & 264 / 18 & 775 & \ci{+0.1799}{+0.1138}{+0.2459} & $+0.1863$ & \textbf{live} \\
mechanism (MoA) & 180 / 26 & 981 & \ci{+0.0663}{+0.0088}{+0.1237} & $+0.0700$ & inconclusive \\
chemical structure & 377 / 94 & 4131 & \ci{+0.0380}{+0.0143}{+0.0617} & $+0.0241$ & inconclusive \\
...
interval's lower bound must clear a pre-set margin of $0.03$: protein target clears it ($+0.1138$),
mechanism ($+0.0088$) and chemistry ($+0.0143$) do not. All three intervals exclude zero, so the two
that fail the margin are inconclusive rather than equivalent to their nulls, and neither may be
written as closed. Two caveats bound every row. The counts are annotations in Tahoe's drug metadata
rather than testable coverage here: within this cache only $18$, $26$ and $94$ of the $94$ scored
drugs received a channel arm at all. And these numbers predate the partner-pool fix --- the pool was
every retained condition in the cell line rather than training conditions only, so a held-out
condition could enter the prediction made for it. The corrected rerun is pending, and no row here
may be read as a training-only retrieval score.}
\label{tab:channels}
\end{table}
...
\midrule
protein target & $54\%$ & $775$ & \ci{+0.1799}{+0.1138}{+0.2459} \\
mechanism (MoA) & $32\%$ & $981$ & \ci{+0.0663}{+0.0088}{+0.1237} \\
chemical structure & $100\%$ & $4131$ & \ci{+0.0380}{+0.0143}{+0.0617} \\
\midrule
\multicolumn{4}{l}{the same three, against the stricter DIFFERENT-PLATE null} \\
protein target & & $688$ & \ci{+0.1115}{+0.0448}{+0.1783} \\
mechanism (MoA) & & $874$ & \ci{+0.0604}{+0.0094}{+0.1114} \\
\textbf{chemical structure} & & $4114$ & $+0.0183$ [$-0.0033$, $+0.0399$] \\
\bottomrule
\end{tabular}
...
removes the last shared batch structure, target and mechanism survive and \emph{chemical structure
does not} --- $+0.0183$ \nolinebreak[3]$[-0.0033, +0.0399]$. Chemistry's apparent signal is a
co-plating artefact. Intervals here are two-way cluster-robust over cell line and treatment well;
...
the field it is already handed, and the answer is that it uses one span and not the other. Swapping
the drug name moves the output by $+0.1026$; swapping the mechanism, with the name left in place,
moves it by $+0.0303$ with an interval containing zero. The name therefore carries about nine tenths
of the combined effect. Both intervals are clustered on the drug and the treatment well; the mechanism
row also fails to clear zero clustered on cell line and well, so the reading does not depend on that
choice. The mechanism row carries fewer conditions by construction: where a swap partner happens to
