...
\texttt{unseen\_combo} (an unseen \emph{well} of a seen drug) and \texttt{unseen\_drug}. Evaluation
...
\midrule
\texttt{revert\_center} (all genes $\rightarrow G/2$) & none & \textbf{0.9999} & undefined \\
...
\ci{+0.635}{+0.618}{+0.652}$, Holm $p = 0.0025$, in the all-plates calibration run ($1{,}820$ drugs
over $25$ cell lines). The four rivals remain strongly negative and none approaches significance
...
stratum. On the identifiable stratum ($n=204$) the model reaches $0.768$: a control-copy
...
generic drug programme, which \S\ref{sec:res-metrics} showed accounts for essentially all of the
model's apparent skill. What remains is what makes this drug different. The inclusion thresholds are
...
\midrule
ceiling (split-half replicate) & a real replicate of this condition & $0.958$ & per split: $0.962$/$0.955$/$0.956$ \\
\texttt{drug\_lookup} (training-only) & this drug's mean residual, other cell lines & $0.941$ & the bar to beat \\
\texttt{drug\_lookup\_1} & the same, from one other cell line & $0.859$ & no averaging advantage \\
model & control cell $+$ drug & $0.569$ & per split: $0.635$/$0.545$/$0.512$ \\
\texttt{moa\_lookup} & drugs sharing this drug's MoA, same line & --- & mechanism-leak diagnostic \\
\texttt{control\_copy} & nothing (predict the control cell) & $0.511$ & drug-agnostic \\
\texttt{generic} & nothing (predict the average response) & $0.500$ & the zero vector \\
...
fitted from training conditions only. \texttt{drug\_lookup} is the pre-registered win condition: a
...
($n = 570$), swapping only the mechanism gave $+0.0091$ [$-0.0174$, $+0.0383$] ($n = 325$), and both
together $+0.1024$ [$+0.0827$, $+0.1236$]. Those arms were scored against the
