...
estimate \ci{+0.635}{+0.618}{+0.652}, all-plates run ($1{,}820$ drugs, $25$ cell lines), and
\ci{+0.519}{+0.479}{+0.557} when partners are restricted to the same plate --- the confound-free
version, lower but still the only calibrated metric of the five; the four
...
outright by \ci{+0.0981}{+0.0407}{+0.1554}, both clustered on the drug, the axis the claim rides on.
...
\begin{tabular}{lrrll}
\toprule
Split & $n$ (lines; drugs) & model $\NIR$ & $\gap$, clustered on \emph{cell line} & the same, clustered on \emph{drug} \\
\midrule
\texttt{train} & 300 (39; 76) & 0.598 [0.549, 0.647] & \ci{+0.0898}{+0.0382}{+0.1415} & \ci{+0.0898}{+0.0291}{+0.1506} \\
\textbf{\texttt{unseen\_combo}} & \textbf{895 (42; 34)} & \textbf{0.533 [0.492, 0.575]} & \ci{+0.0332}{+0.0069}{+0.0594} & $+0.0332$ [$-0.0031$, $+0.0695$] \\
...
What the slope on \emph{unseen drugs} adds is the sharper reading: at
\ci{+0.342}{+0.215}{+0.469} the model tracks the named drug just as well on compounds held out of
fine-tuning entirely. Re-encoding did not teach the model about drugs. It changed the target so that
drug knowledge the model already had could reach the output.
\end{claim}
...
\texttt{moa\_lookup} ($n = 118$) & drugs sharing this drug's MoA & $0.565$ & mechanism-leak check \\
...
cell's state can beat it. \texttt{moa\_lookup} is scored on the $118$ conditions that have a
training-drug MoA partner, so that row does not sit on the $n = 570$ support the rest of the ladder
does. The paired comparison on the common support ($n = 449$) is model $-$
...
here: the model does beat \texttt{moa\_lookup} on the $118$ conditions where that comparison exists,
$\gap = \ci{+0.0413}{-0.0610}{+0.1436}$ two-way clustered, but the interval spans zero, so the
comparison is uninformative rather than favourable.
