...
correct $+0.1429$); the two controls were \emph{not} re-run at that budget, so the three-column
comparison is not budget-matched and is not presented as one. Both control arms are scored at
$600$ tokens against the residual arm's $1400$. Truncation costs the residual arm signal rather
than granting it any, so the mismatch works against the effect being reported rather than for it,
but the comparison should be read as unmatched on generation budget until those arms are re-run. And the optimal-transport
