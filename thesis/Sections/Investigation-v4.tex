...
that came before it, so a methods chapter would describe tools whose motivation lives three sections
away, and a results chapter would quote numbers whose meaning depends on definitions met fifty pages
earlier. Instead, each part below poses a question, defines the instrument built to answer it,
reports what the instrument showed, and closes with what that showing forced next. Design choices
are stated as the failures they avoid. Definitions appear where they are first needed, and each
section closes with its central claim set off from the text. Two conventions hold throughout: every
number is quoted with its interval where one exists, and ``not established'' and ``shown to be
zero'' are different statements --- the difference is load-bearing in several places.
