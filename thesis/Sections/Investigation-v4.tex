...
a prompt must carry an instruction line and two cells, so the full transcriptome does not fit. Neither
step changes the rank sentence, and it is worth being exact about that because it is easy
...
occurring in exactly one well. That last figure is a property of the three plates this project
caches (\texttt{plate4}, \texttt{plate5}, \texttt{plate6}) and not of the atlas: Tahoe includes
further plates, among them a replication plate, and where the text below says a treatment occurs in
one well it means one well \emph{here}. No cross-plate replicate of a (drug, dose) treatment exists
within this cache, which is what the inference rules below depend on; conditions that do appear on
more than one of these plates differ in dose. Each drug is assayed on its own plate or plates, roughly $4$--$12$
...
because this cache places $286$ of $287$ treatments in exactly one well --- a biological replicate does
...
three frames below score with a distance rather than a similarity, and there $\sigma$ is its
negation --- $\sigma(\hat y, y) = -\lVert \hat y - y\rVert_2$ for \texttt{nir\_expr} --- which is
what reverses the inequality in the implementation. The construction is otherwise identical across
frames; only $\sigma$ and $J$ change, and both are declared wherever a figure is quoted. The comparison set is declared at every score: conditions are grouped
...
figure in this chapter is \texttt{nir\_expr}, and that choice is load-bearing.
...
One consequence of that has to be discharged rather than assumed. Everything above grades
\texttt{nir\_expr}; the residual chapter scores a different $\sigma$ over a different comparison set,
and a metric is not calibrated by having a relative calibrated. The residual frame therefore carries
its own $\DRF$, computed the same way with its own controls: the positive control is a split-half
replicate of the condition and the negative controls are the drug-agnostic arms already in the
evaluation. It reads $\DRF = +0.704$ $[+0.667, +0.736]$ against \texttt{control\_copy}, clustered on
the $42$ cell lines, and the figure is insensitive to which drug-agnostic arm is used as the
negative: $+0.707$ against a random vector and $+0.708$ against the neutral scrambled partner, all
three of which sit at chance empirically ($0.506$, $0.501$, $0.501$) rather than by stipulation. The
residual metric is thus better separated than the expression-frame one ($+0.635$), which is the
direction the re-encoding was intended to move. Two caveats travel with it. The positive control is a
raw split-half replicate rather than the denoised interpolated duplicate the expression-frame
protocol uses, so the figure is conservative by comparison; and the \texttt{generic} arm, which
scores exactly $0.500$ with zero variance because the residual is defined by subtracting it, is
reported only for completeness --- using it would make $\DRF$ a rescaling of the ceiling and test
nothing.
...
carries identity; hence the contrast score, in which the position-uniform part of that inflation
cancels and the \texttt{permuted} arm absorbs the rest.
...
and $2$ excludes zero on its own, so the dose--response is real ($+0.0013$, $+0.0017$, $+0.0044$),
...
robust to that choice. This result is carried as the weakest load-bearing element in the chapter,
no argument downstream rests on it alone, and a reader who discards it entirely loses nothing that
the drug-blindness result depends on.
...
\begin{claim}
There is a conditional target that a per-drug constant cannot express, and no learnable structure in
it is found. Its size is quoted as what it is: $\Tcoef = 0.553$ \nolinebreak[3]$[0.514, 0.593]$ is a
...
unchanged retrieval ceiling, and produces the first non-null result in the project --- first in
sequence, and, as the budget-matched re-run below shows, not the only target construction that
produces one. On the earlier
...
stratum ($-0.001$ to $+0.016$) \emph{at the $600$-token budget those controls were scored at}. The
optimal-transport arm does not stay flat once it is given the residual arm's budget, as the
re-run below establishes; the single-cell arm does. These magnitudes were read against the \texttt{opposite} comparator,
...
correct $+0.1429$). Both controls have since been re-run at the residual arm's $1400$-token
...
What follows from that has to be said plainly, because it costs this section its strongest
phrasing. The residual arm's own pooled gap against the neutral comparator is
$+0.0361$ \nolinebreak[3]$[+0.0159, +0.0564]$, and the optimal-transport arm's is
$+0.0292$ \nolinebreak[3]$[+0.0144, +0.0441]$. Those intervals overlap almost entirely. Re-encoding
the target therefore produces prompt sensitivity, but it is \emph{not} the only target change that
does, and the claim that it produced the first non-null effect in the project holds only in the
order the experiments were run, not as a statement that the residual encoding is uniquely
responsible. Two limits bound the comparison in the other direction: the arms are scored in
different frames, so their absolute $\NIR$s are not comparable and only each arm's gap against its
own comparator is; and they are separate evaluations over different condition sets
($1{,}394$ against $248$), so this is an arm-level comparison and not a paired one. And the optimal-transport
...
\begin{claim}
Re-encoding the target produced the project's first non-null drug effect, though a
budget-matched re-run of the optimal-transport control shows it is not the only target construction
that produces one. On trained
...
lookup's $115.7\%$. That pooled figure should not be the one quoted, because the lookup is not
...
conclusion does not turn on the choice, but the held-out number is the one that means what the
sentence says.}
\label{tab:baselines}
\label{tab:lookup}
\end{table}
...
which $14$ are held out of training entirely --- and the target channel's arm covers $18$ drugs, of
...
target channel survives the restriction at all, and it survives on two drugs. What this section measures is therefore
...
channel and which mechanism does not survive at all. Every verdict below should be read in those
terms, and the section's title is a description of the question rather than of what was measured.
...
clusterings, but its clearance of the relevance margin holds on the cell-line axis and not on the
drug axis, and rests on $18$ drugs. This is the same axis question \S\ref{sec:res-transfer} settles
...
draft of this claim, and narrower again once the clustering axis is chosen by the claim rather than
by the gate. Protein target is the strongest channel by a clear margin and excludes zero on every
construction under both clusterings; it clears the pre-set relevance margin of $0.03$ clustered on
cell line and well, but not clustered on the drug, which is the axis a statement about unseen drugs
rides on, and it rests on $18$ annotated drugs of which two are held out of training. Mechanism
...
conditioning. Chemistry's interval is the
narrowest of the three and the margin falls inside it. None of the three may be written as closed,
and none may be written as live without naming the axis. Similarity transfer through the target
channel is measurable and is the only channel worth pursuing; whether it is large enough to matter
is unresolved at $18$ drugs.
\end{claim}
...
split-half figure, not a biological replicate: this cache places $286$ of $287$ treatments in exactly one
