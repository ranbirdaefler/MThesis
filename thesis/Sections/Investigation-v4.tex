...
training target as the drug-specific residual makes the model demonstrably sensitive to the drug in its prompt --- yet that sensitivity
...
\emph{from the other drugs on its own plate and cell line}, and that qualifier does work.
...
text below gives, is why this row is reported as not established. Only
...
\midrule
\textbf{protein target} & 264 / 18 & 712 & \ci{+0.1351}{+0.0745}{+0.1958} & $+0.1498$ & \textbf{live}$^{\dagger}$ \\
...
margin simply falls inside it. $^{\dagger}$The target verdict is bolded here because it is the
gate's own, and the gate clusters on cell line and well. It does \emph{not} survive clustering on
the drug, which is the axis a claim about unseen drugs has to generalise across, and this section
concludes that none of the three may be written as live without naming the axis
(\S\ref{sec:res-scope}); read the row with that qualification and not without it. One further
caveat bounds every row. The counts are annotations in Tahoe's
