...
family was in fact tested. Both are repaired in the analysis code, and the all-plates arm has been
rerun under the repair; the same-plate arm has not, so its point estimates are quoted below and its
interval is not.
...
errors from zero. Restricting swap partners to the same plate ($335$ drugs over the same $25$ cell
lines) pulls every value toward zero and changes no sign: $\NIR$ $+0.519$, against $-0.122$,
$-0.247$, $-0.495$ and $-0.500$ in the same order. That arm has not been rerun under the repaired
bootstrap, so its point estimates are quoted and its interval is not. What the argument uses is the
sign pattern, which is identical in both runs; no ranking among the four negative metrics is
claimed. The inversion is a
...
Intervals are drawn for the all-plates arm only: the same-plate bootstrap still treats each drug row
as its own cluster, so its point estimates stand and its intervals are withheld (see text). \texttt{weighted\_r2} carries no directional claim: its sign tracks cell count.}
...
$[+0.604, +0.663]$ clustered on the $25$ cell lines, Holm $p = 0.002$; $+0.519$ as a point estimate
with same-plate partners, an arm whose interval is withheld pending its own re-run); and on $\NIR$
...
mechanism ($+0.0279$) and chemistry ($+0.0035$) do not. All three intervals exclude zero, so the two
