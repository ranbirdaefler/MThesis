...
a treatment occurs in one well it means one well \emph{here}. No cross-plate replicate of a (drug, dose) treatment exists
within this cache, which is what the inference rules below depend on; conditions that do appear on
more than one of these plates differ in dose. Each drug is assayed on its own plate or plates, with
...
None is a biological replicate: this cache places $286$ of $287$ treatments in a single well, so for
those there is no second well to replicate against. In the residual frame the reference is not
constant across splits ($0.956$ / $0.824$ /
...
is quoted, together with the budget it was generated under. The generation configuration itself is
fixed and is recorded here because the intervals do not cover it: sampling at temperature $0.8$ with
top-$p$ $0.9$, four samples per condition in the residual-frame evaluations of
\S\ref{sec:res-encoding} and their control re-runs, and eight in the expression-frame evaluations
that feed \S\ref{sec:res-blind} and Table~\ref{tab:epsilon}. Every
...
carries. Measuring those is not the same as showing that the target encoding is the defect, because
the residual construction changes aggregation, control referencing, sign coding and target length
...
near-constant across the three rows; what the target encodings differ in is the first column ---
...
unchanged within-well split-half retrieval reference, and the model trained on it responds to the
drug it is told about. On
...
against the neutral \texttt{orth} partner, the only comparator that can carry a magnitude claim; the
\texttt{opposite} figures are quoted in prose only, as the diagnostic, never as a column. The
...
ladder does; paired on that support model $-$ \texttt{moa\_lookup} $= +0.0436$
