...
the discarded magnitude not to matter for downstream use. The criterion is absolute rather than
discriminative: reconstruction fidelity asks how close the
...
against $0.533$ on the $893$ conditions of the held-out wells, where the lookup cannot
...
The result separates from Miller et al.\ in two places, and only the first is a metric-level
disagreement. That disagreement begins with weighted $R^2\Delta$, which Miller et al.\ endorse as
calibrated but which is negative in both arms under our implementation. Its sign tracks cell count,
so Section~\ref{sec:res-metrics} draws no directional conclusion from that negative, but this member
of their calibrated set does not transfer intact. Their endorsed rank-based metric, $\NIR$, does
survive. A broader observation about rank is separate from that comparison: $\paneltau$ and
\texttt{spearman\_expr}, neither a member of Miller et al.'s calibrated set, both invert in this
field-wide audit. Their failure does not generalise to $\NIR$ or to rank-based metrics as a family.
The second separation is the conclusion about models. Miller
et al.\ report that deep models beat uninformative baselines once the metric is fixed; on the metric
their analysis endorses, the checkpoint evaluated here does not
(Section~\ref{sec:res-blind}). Both results can stand: a repaired metric is a precondition for
seeing skill, not a guarantee that there is skill to see. Which difference in setting accounts for
the split --- genetic against chemical perturbation, a discriminative regression model against an
autoregressive generative one, or the strength of the per-drug baseline in this atlas --- is taken
up where the measurements are, in Sections~\ref{sec:res-metrics}
and~\ref{sec:res-lookup}.
...
holdout maps onto one of these and deliberately misses another. \texttt{unseen\_drug} is
Leave-Drugs-Out. \texttt{unseen\_combo} is \emph{not} Leave-Pairs-Out, and
\S\ref{sec:methods-holdout} uses that name only for the estimand it declines: holding out a
(drug, cell line) pair leaves the pair's own treated well in training through its other
$\approx 48$ cell lines, which measures within-atlas matrix completion rather than generalisation.
Our split withholds the whole treatment well instead, so it is strictly the stronger of the two and
answers a narrower question --- an unseen well of a seen drug. The mapping carries a
...
it on condition-level pseudobulk residuals built from single cells, with attenuation from within-well
sampling noise corrected, and what comes back is a number rather than a yes or a no. On a
disattenuated cross-context similarity scale the
shared part is \ci{\Tcoef = 0.553}{0.514}{0.593}: a drug's residual signature in one context
recovers a little over half of itself in another, far above the structure-matched cross-line
negative control ($-0.008$) and far below reproduction. The earlier same-plate, different-drug
control ($-0.017$) is not a second matched control: it differs structurally from the estimand and was
voided on that basis. The remaining \ci{44.7\%}{40.7\%}{48.6\%} is not thereby shown to be biology.
The compared pairs differ in cell line, in dose and in treatment well at once, so the figure is not
a variance share and not a drug$\times$cell-line interaction, and the section says so where it
reports it. What the measurement delivers is a calibration of the transfer premise rather than a
verdict on it, and we are aware of no prior estimate of that quantity for chemical single-cell
perturbation data at condition-level resolution.
...
$\varepsilon \rightarrow 0$ the plan concentrates toward an unregularised sparse coupling; without
further conditions, that limit need not send each control cell to one observed treated cell. As
$\varepsilon$ grows the plan blurs, and in the limit every control cell is coupled equally to every
treated cell --- the product of the marginals. Given a coupling, the \emph{barycentric projection}
maps each control cell $x_i$ to the $\pi$-weighted average of its partners,
\begin{equation}
  \hat{y}(x_i) \;=\; \frac{\sum_j \pi_{ij}\, y_j}{\sum_j \pi_{ij}},
\end{equation}
which is a convex combination inside the observed treated convex hull, not necessarily a point on
the data manifold. The two extremes of $\varepsilon$ therefore produce a target dominated by a
sparse set of observed treated cells at small $\varepsilon$, and the group mean at large
$\varepsilon$. A sweep over $\varepsilon$ is thus a continuous interpolation between two target
constructions that would otherwise have to be treated as unrelated alternatives, which is how
Section~\ref{sec:res-epsilon} uses it.
...
behave. And because the cost of a full coupling grows quadratically, transport is solved separately
for each treatment condition. Each plan couples that condition's treated cells to the controls
shared within its cell-line/plate group; it neither pools treated drugs into one group-level coupling
nor couples across the atlas. This is both computationally necessary and biologically correct, since
a control cell from one cell line is not a plausible counterfactual partner for a treated cell from
another.
