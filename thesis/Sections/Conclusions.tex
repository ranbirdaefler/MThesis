...
the cell-line channel is clustered, so they are robust on that axis alone. This is the most portable result
...
\ci{+0.0263}{+0.0069}{+0.0476} on the \texttt{opposite} stratum, though not on the neutral one ---
but that arm ran at a $600$-token generation budget against the residual arm's $1400$ and was not
recomputed in the canonical run, so \S\ref{sec:res-epsilon} declines to put the two into ratio and
so does this sentence.) Re-encoding
the target as the drug-specific residual raises the differing fraction to $59\%$ and produces the
first non-null drug effect in this project --- first in sequence rather than uniquely, since a
budget-matched re-run of the optimal-transport control also clears zero --- with a monotone gradient across swap dissimilarity and a
...
chemistry carries a small one. None of the three may be written as closed, and only target may be
written as live.
...
unseen drugs, where retrieval is impossible by construction, protein-target annotation is the one
channel carrying signal against a different-plate null with the partner pool restricted to training
drugs, and is therefore where a model may be able to do something a dictionary cannot --- with the
size of that opening still unresolved, because on the drug axis its interval clears zero but not the
relevance margin, on $18$ drugs. That
