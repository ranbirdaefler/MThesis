...
\paragraph{And the drug use appears to generalise.} On a holdout keyed at the (drug, cell line)
level, pairings never seen in training score \ci{+0.1002}{+0.0661}{+0.1368}, indistinguishable from
trained conditions, with held-out \emph{drugs} null as the control predicts. The natural reading is
a drug representation that transfers rather than memorised pairings. It is stated as a reading
rather than a result because the targets were built with the generic fitted over the holdout as
well as the training set, and because a held-out pairing can share a treated well with a training
condition of a different cell line. The rebuilt targets settle both; this thesis reports the
number and the two reasons to hold it loosely.
