...
though each row were its own cell line ($1{,}820$ and $335$ rows, when there are $25$ cell lines),
so their intervals were far too narrow. The across-plates arm has been rerun with the bootstrap
clustered on cell line and the null imposed: $\DRF(\NIR) = +0.635$, $[+0.604, +0.663]$, Holm
$p = 0.002$ across the family of five. That $p$ is still at the resampling floor, but now because
the estimate sits some forty cluster-robust standard errors from zero rather than because each row
was counted as its own cluster. The within-plate arm has not been rerun, so its $+0.519$ is quoted
as a point estimate without an interval. This is the most portable result
