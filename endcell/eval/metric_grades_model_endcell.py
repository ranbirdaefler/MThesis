...
def rankvec(sentence, panel, worst):
    # Strip the [END_CELL] sentinel (present in the endcell data format) before ranking;
    # it is a marker, not a gene, and must not occupy a rank position.
    if "[END_CELL]" in sentence:
        sentence = sentence.replace("[END_CELL]", " ")
