...
- **Answer — the readout is NOT drug-specific; the drug is increasingly encoded but never read for identity.**
  ...
  **What survives unaffected:** the ablation ratios (their null, `V_rand_pure`, was already a matched-*dimension* random subspace — a different and correctly-constructed control), the variance shares, and the decodability-versus-depth dissociation. **What does not:** the direction-blindness claim specifically, which rests entirely on the swap.
  ...
  1. **The swap is null at every layer (headline).** ⚠️ **RETRACTED — see the confound above and Q21.** Overwriting drug A's code with drug B's perturbs the output *no more than — actually less than — matched-norm noise*, at all 7 depths. The readout responds to the *magnitude* of activity in the drug subspace, not to *which drug* it encodes. This is the mechanistic statement of drug-blindness, and it is confound-clean and depth-invariant.
...
### Q21. Does the generation readout read drug IDENTITY? (Q13 redone on a control that survives audit, then replicated)
- **Why:** Q13's headline — "swapping drug A's code for drug B's moves the output *less* than matched noise" — was retracted **twice**, for the same species of defect both times. v1 drew the matched-norm random vector in the full 2048-dim space while the swap vector lived in a ≤10-dim ablated slab. v2 drew it inside the slab, which looked like a fix and was not: the hook does **ablate-then-add**, so the delivered perturbation is `(added − removed)` while only `added` was norm-matched, and because `drug_mean` was the **raw uncentered** class mean the injected vector and the removed component shared the global mean. The swap arm put back roughly what it removed; the random arm put back something orthogonal. Simulated with **no model in the loop**, the random arm delivers **5.3× the perturbation in 100% of draws**, with predicted gap `2⟨c_B,s⟩ = 9.179` against observed `9.162`. The retracted result was geometry, not biology. A third design — `Δ logP(r_B)` against a norm-matched random direction — was confounded the *other* way: an off-manifold control inflates the logsumexp (+2.75 nats measured) while an on-manifold class-mean displacement does not, and with the scored target stripped of all identity information **36% of the effect still survived**. Norm-matched is not disruption-matched.
- **How (the design that survived):** the identity test no longer ablates. It injects a **displacement** and scores a **contrast**, so that the arms differ in nothing but which drug they name.
  - **Score.** `logP(r_B | prompt_A) − logP(r_A | prompt_A)`, where `r_B` is a real held-out response emitted under drug B. Any shift common to both responses — including the entire normaliser — cancels algebraically. What survives is the model's *preference* between B's response and A's own.
  - **Arms, all anchored at drug A** so every arm is an on-manifold class-mean displacement sharing the same cross term with `h`: `swap = P(μ_B − μ_A)`; `permuted = P(μ_C − μ_A)` (**the headline control** — same anchor, same construction, same norm, wrong drug); `isotropic` = random in slab, retained only as an off-manifold **damage** diagnostic. `μ_B − μ_A` cancels the global mean exactly.
  - **Everything out of sample.** Subspaces *and* class means fit on an est slice; `r_B` drawn from a **third disjoint slice**, never from the rows defining `μ_B`; all scoring on the eval slice. v2 held out only the means, leaving the subspace — the object the claim is about — fit on the rows it was then measured on.
  - **Guards.** Cluster bootstrap over (A,B) pairs; TOST equivalence requiring the CI to sit inside the margin **and** contain zero; a per-layer headroom gate excluding layers where the cell-line positive control has no dynamic range; a drug-proxy screen refusing any context label that is a near-bijection with drug (this caught `metadata["dose_float"]` holding a **sample ID** in the residual/OT builders — had it merely parsed, orthogonalising against it would have deleted the drug axis by construction); BH across depths; a magnitude ladder α ∈ {0.5,1,2,5,10}; and an injected-norm diagnostic reported against the **centred** slab component (0.69–1.40, i.e. the injection is the size of the thing it displaces).
  - **Selftest with teeth.** Three planted worlds — drug inert, drug read, and drug inert behind a **norm-sensitive** readout — run through the real `run_swap`. Separation −0.042 / +1.520 / −0.010. It failed four times during development and each failure was a real defect.
- **Answer — the readout is low-gain, not categorically blind, and the arms dissociate.**
  Five arms (`single_cell`, `consensus`, `ot_T2`, `residual`, `residual_holdout`) × up to 7 layers = 26 headline tests. **Exactly one survives BH correction:** `residual` at **layer 12**, `+0.0197` of that model's own clean A-vs-B preference gap, `[+0.0063,+0.0335]`, p=0.0010, **q=0.026**. It carries a monotone dose-response (detected at α = 0.5, 1, 2: +0.0013, +0.0017, +0.0044). `consensus` layer 2 (p=0.012) does **not** survive (q=0.156).

  **Replicated** (`probereplicate.sbatch`, criterion fixed in writing beforehand: positive with CI excluding zero in ≥2 of 3 new seeds):

  | seed | effect | 95% CI | p |
  |---|---|---|---|
  | 42 (original) | +0.0197 | [+0.0063, +0.0335] | 0.0010 |
  | 43 | +0.0118 | [+0.0060, +0.0173] | 0.0003 |
  | 45 | +0.0085 | [+0.0029, +0.0143] | 0.0015 |

  Mean **+0.0133** over the three seeds that measured it. Seed 44 produced **no layer-12 measurement** (it lost 5 of 7 layers), so the criterion is met 2-of-3 with the third a non-measurement rather than a failure. `single_cell` shows nothing at **any** of six layers (median −0.0012).
- **Verdict:** **the instrument is validated on real data, and it dissociates the arms.** The probe detects drug-identity reading in the model independently known to use the drug (residual, +0.143 NIR) and detects none in the model known to ignore it. That is the positive control the two retracted versions never had. The mechanistic claim is therefore **not** "the readout cannot read drug identity" but "**the readout reads it at a gain far too low to matter**" — ~1% of the preference gap, an order of magnitude below the 10% equivalence margin, in the arm where it is detectable at all.
- **Caveats (state, don't hide):** (a) **one layer of seven.** The effect is at layer 12 and nowhere else; layer 4 is positive in seed 45 alone and null in 42/43/44, which is what multiplicity discipline is for. (b) **`single_cell` has no layer-12 measurement** — excluded there for instrument saturation (3.88×, under the 5× gate) — so the cleanest comparison, same layer across two arms, does not exist; the dissociation rests on `single_cell` being null at all six layers it *could* measure. (c) **the sibling arm does not reproduce it**: `residual_holdout` is null at layer 12 in the two seeds where it was measured (+0.0028, +0.0049). (d) **measurement availability is seed-dependent** — seed 44 scored only layers 2 and 4 — which is itself worth understanding before the probe is reused. (e) the effect is small in absolute terms and sits below the pre-registered equivalence margin, so it is a *detection*, not a magnitude worth building on.
- **Status:** ✅ **Q13's suspension is lifted, and its headline is replaced rather than restored.** The retracted claim (swap perturbs *less* than noise) was an artifact and does not come back. What replaces it is weaker, better controlled, and replicated: an increasingly explicit drug representation that the generation readout consults at negligible gain. Raw logs: `logs/probearms_*.out`, `logs/probereplicate_*.out`; JSON: `RESULTS/probe_arm_*.json`, `RESULTS/probe_rep_*.json`; script `endcell/analysis/workspace_probe.py` (v3).

### Q22. Where did the data defects come from, and what do they invalidate? (source-level audit of the residual pipeline)
- **Why:** an external audit flagged three problems in the residual arm — a `dose` column holding sample identifiers, a transductive holdout, and an oracle lookup baseline. All three were verified personally. This entry records what the *sources* turned out to be, because the origin decides the cost of the repair and, in one case, decides that a claim is unaffected rather than broken.
- **The sample identifier.** `build_embeddings.py:134` read `row.get("sample")` and appended it to a metadata column named `dose`. That single line is the whole origin of `metadata["dose_float"] == "smp_1841"`. The consequence is that the sample identity was **mislabelled, not lost**: every downstream "same dose" and "different dose" statistic was a statement about well identity, but the well identity itself is recoverable from the existing cache. The cache is therefore **enriched in place**, not rebuilt — this was the largest cost uncertainty in the remediation and it resolved to the cheap branch. Fixed at the source (`sample_id`), and both readers recover it through `tahoe_design.sample_column`, which reports which branch it took.
- **A second, independent dose defect.** `tahoe_c2s_preprocess_endcell.py:970-972` computes `dose_float = float(dose_str.split()[0])`, discarding the unit. Two failure modes follow and they are opposite: **collision** (1 uM and 1 nM both become 1.0, so two concentrations are treated as one) and **split** (0.05 uM and 50 nM are one concentration but become 0.05 and 50.0). Tier 4 — the held-out-dose test — is decided at `:1006-1008` by `abs(dose_float - held_out_dose_per_drug[drug]) < 1e-6`, i.e. by exactly that comparison, so a collision means tier 4 held out a dose it also trained on. Whether either mode actually bit is a property of the data and is measured, not assumed, by `experimental_unit_audit.py` question 5b. Note that this is a **different** defect from the sample-ID one and lives in a **different** pipeline: the original preprocessing writes the well identifier correctly, under `metadata["sample"]`.
- **The leak.** `build_residual_targets.run()` fitted the generic shift and the reliability filter over every condition and assigned the train/holdout split afterwards, so each held-out target was defined partly by the other held-out conditions. Now `inventory -> split -> fit -> transform`, with the split assigned from metadata before a single pseudobulk exists. The gate is `tests/test_split_before_fit.py`, a **poison test**: corrupt every held-out expression vector, rebuild, require the training JSONL and a digest of every fitted quantity to be byte-identical. Restoring the one-line defect makes it fail on the digest, so it is not vacuous; poisoning a *training* condition instead must change both, which is the negative control.
- **Two defects found in the same function while fixing the leak.** The generic was **cell-line-scoped**, which leaves same-plate structure in the target at +0.478 against −0.018 at plate scope — and the shipped targets were built the contaminated way. And it was a mean over **conditions**, so a drug measured at five doses carried five times the weight of a single-dose drug. Both are repaired, with **leave-one-drug-out** added so a train condition is not centred on a mean containing itself while a held-out condition is.
- **What this does NOT invalidate.** The κ transfer coefficient's *dose* arm is void (it compared sample identifiers) and is being recomputed, but the metric-calibration results (Q10), the OT ε-ladder, and Q21's probe do not route through the residual holdout or through any dose comparison. Q21's drug-proxy screen is what **caught** the sample-ID defect in the first place: it refused `dose_float` as a context label because it was a near-bijection with drug. Had that column merely parsed as a number, orthogonalising against it would have deleted the drug axis by construction and the probe would have returned a confident null.
- **The unavoidable residue.** A (drug, cell_line) holdout **cannot** stop a held-out condition sharing a treated well with a training one, because one Tahoe well carries many cell lines. This is a property of the experimental design, not of our code, so it is counted and reported in the holdout manifest rather than hedged in prose. `--split_unit sample` drives it to zero but changes the estimand to dose/replicate generalisation.
- **Status:** 🔧 **repairs landed, numbers pending.** Commits `762951a` (design layer), `8525254` (unit audit), `41c6716` (split-before-fit + unit). The corrected numbers may move in **either** direction — the leak and the plate structure inflate the shipped residual result, while the oracle `drug_lookup` and the same-drug scramble partners deflate it — so no wording should be drafted before `rebuild.sbatch` and `retrain.sbatch` land.
