...
- **Status:** ✅ **the drug-blindness is a readout-specificity failure, not an encoding failure.** Supersedes Q7 (the inconclusive steering probe). Corroborates the "read but not used" spine (Q6 + Q4) and gives it a mechanism. ➡️ motivates **Arm 1b** (objective-side discrimination loss: contrastive / swap-consistency / KL-maximizing auxiliary that rewards the readout for distinguishing drug directions). One confirmatory job queued: the **held-out-means swap** (`workspace_probe.py` updated; re-run `workspace_probe.sbatch`). Raw log (within-sample swap): `logs/workspace_probe_596144.out`; JSON: `RESULTS/workspace_probe.json`.
